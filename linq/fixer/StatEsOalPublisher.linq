...
async Task Main()
{
	...
	var reports = new List<StatReport>();
	...
	foreach (var branch in branches)
	{
		...
		var esStatList = await GetEsStat(elasticSearchUrl, indexName, branch.Key, checkDate);
		//esStatList.Dump();

		var toDateOalDeliveryList = GetToDateDeliveryEventNew(branch.Key, checkDate);
		var diff2 = esStatList.Except(toDateOalDeliveryList, new StatComparer());
		diff2.Dump();

		reports.Add(new StatReport { BranchCode = branch.Key, BranchName = branch.Value, TotalEs = esStatList.Count, TotalOal = toDateOalDeliveryList.Count, MissingToDate = diff2.Count() });
		...
		foreach (var stat in diff2)
		{
			var date = DateTime.ParseExact(stat.Date, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture);
			var time = DateTime.ParseExact(stat.Time, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture);

			var fileName = string.Format("sa_stat_{0:yyyyMMdd}_{1:HHmmss}_{2}.txt", date, time, stat.ScannerId);
			var data = string.Format("{0}\t{1}\t{2}\t{3:ddMMyyyy}\t{4:HHmmss}\t{5}\t{6}\t{7}\r\n",
					stat.CourierId, stat.LocationId, stat.BeatNo, date, time, stat.ConsignmentNo, stat.StatusCode, stat.Comment);

			//Console.WriteLine(data);
			sb.Append(data);
			await PostStatData(rtsClient, fileName, data, productionToken);
		}
		var fn = string.Format("sa_stat_{0}_{1}_{2:yyyyMMdd}.txt", DateTime.Now.ToString("yyyyMMdd_hhmmss"), branch.Key, checkDate);
		File.AppendAllText(@"R:\script sa file\filedeli\Stat\" + fn, sb.ToString());
	}
...
private async Task PostStatData(HttpClient client, string filename, string data, string token)
{
	...
	var response = await client.PostAsync("stat", request);
...
private async Task<List<Stat>> GetEsStat(string elasticSearchUrl, string indexName, string officeNo, DateTime date)
{
	...
	var response = await client.PostAsync("stat/_search", request);
	...
	if (response.StatusCode == System.Net.HttpStatusCode.OK)
	{
		...
		foreach (var stat in hits)
		{
			list.Add(new Stat
			{
				CourierId = stat["_source"]["CourierId"],
				LocationId = stat["_source"]["LocationId"],
				BeatNo = stat["_source"]["BeatNo"],
				Date = stat["_source"]["Date"],
				Time = stat["_source"]["Time"],
				ConsignmentNo = stat["_source"]["ConsignmentNo"],
				StatusCode = stat["_source"]["DestOffice"],
				Comment = stat["_source"]["Comment"],
				ScannerId = stat["_source"]["ScannerId"]
			});
		}
	}
	return list;
...
private List<Stat> GetToDateDeliveryEventNew(string officeNo, DateTime checkDate)
{
	...
	var sql = @"SELECT DISTINCT [consignment_no], [courier_id] FROM [dbo].[status_code_event_new] WHERE office_no = '{0}' AND date_field > '{1:yyyy-MM-dd HH:mm:ss}'";
	...
	using (var cmd = new SqlCommand(query, conn))
	{
		...
		using (var reader = cmd.ExecuteReader(CommandBehavior.CloseConnection))
		{
			while (reader.Read())
			{
				list.Add(new Stat { ConsignmentNo = reader.GetString(0), CourierId = reader.GetString(1) });
			}
		}
	}
	return list;
...
public class StatComparer : IEqualityComparer<Stat>
{
