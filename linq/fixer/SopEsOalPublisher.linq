...
async Task Main()
{
	...
	var checkDate = new DateTime(2017, 8, 22);
	Console.WriteLine("{0:dd-MM-yyyy}\r\n========", checkDate);

	var branches = GetAllBranch();
	var reports = new List<SopReport>();
	...
	foreach (var branch in branches)
	{
		...
		var esSopList = await GetEsSop(elasticSearchUrl, indexName, branch.Key, checkDate);
		//esSopList.Dump();

		var toDateOalDeliveryList = GetToDateDeliveryEventNew(branch.Key, checkDate);
		var diff2 = esSopList.Except(toDateOalDeliveryList, new SopComparer());
		//diff2.Dump();

		reports.Add(new SopReport { BranchCode = branch.Key, BranchName = branch.Value, TotalEs = esSopList.Count, TotalOal = toDateOalDeliveryList.Count, MissingToDate = diff2.Count() });
		...
		foreach (var sop in diff2)
		{
			var date = DateTime.ParseExact(sop.Date, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture);
			var time = DateTime.ParseExact(sop.Time, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture);

			var fileName = string.Format("sa_Sop_{0:yyyyMMdd}_{1:HHmmss}_{2}.txt", date, time, sop.ScannerId);
			var data = string.Format("{0}\t{1}\t{2}\t{3:ddMMyyyy}\t{4:HHmmss}\t{5}\t{6}\t{7}\t{8}\r\n",
					sop.CourierId, sop.LocationId, sop.BeatNo, date, time, sop.ConfirmLocation, sop.ConsignmentNo, sop.DestOffice, sop.Comment);

			//Console.WriteLine(data);
			sb.Append(data);
			await PostSopData(rtsClient, fileName, data, productionToken);
		}
		var fn = string.Format("sa_sop_{0}_{1}_{2:yyyyMMdd}.txt", DateTime.Now.ToString("yyyyMMdd_hhmmss"), branch.Key, checkDate);
		File.AppendAllText(@"R:\script sa file\filedeli\Sop\" + fn, sb.ToString());
	}
...
private async Task PostSopData(HttpClient client, string filename, string data, string token)
{
	...
	var response = await client.PostAsync("sop", request);
...
private async Task<List<Sop>> GetEsSop(string elasticSearchUrl, string indexName, string officeNo, DateTime date)
{
	...
	var response = await client.PostAsync("sop/_search", request);
	var list = new List<Sop>();
	...
	if (response.StatusCode == System.Net.HttpStatusCode.OK)
	{
		...
		foreach (var sop in hits)
		{
			list.Add(new Sop
			{
				CourierId = sop["_source"]["CourierId"],
				LocationId = sop["_source"]["LocationId"],
				BeatNo = sop["_source"]["BeatNo"],
				Date = sop["_source"]["Date"],
				Time = sop["_source"]["Time"],
				ConfirmLocation = sop["_source"]["ConfirmLocation"],
				ConsignmentNo = sop["_source"]["ConsignmentNo"],
				DestOffice = sop["_source"]["DestOffice"],
				Comment = sop["_source"]["Comment"],
				ScannerId = sop["_source"]["ScannerId"]
			});
		}
	}
	return list;
...
private List<Sop> GetToDateDeliveryEventNew(string officeNo, DateTime checkDate)
{
	...
	var sql = @"SELECT DISTINCT [consignment_no], [courier_id] FROM [dbo].[sop_event_new] WHERE office_no = '{0}' AND date_field > '{1:yyyy-MM-dd HH:mm:ss}'";
	...
	using (var cmd = new SqlCommand(query, conn))
	{
		...
		using (var reader = cmd.ExecuteReader(CommandBehavior.CloseConnection))
		{
			while (reader.Read())
			{
				list.Add(new Sop { ConsignmentNo = reader.GetString(0), CourierId = reader.GetString(1) });
			}
		}
	}
	return list;
...
public class SopComparer : IEqualityComparer<Sop>
{
	public bool Equals(Sop x, Sop y)
	{
		return x.ConsignmentNo == y.ConsignmentNo;
	}

	public int GetHashCode(Sop x)
	{
