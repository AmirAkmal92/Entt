...
async Task Main()
{
	...
	foreach (var branch in branches)
	{
		...
		//var diff = esDeliveryList.Except(oalDeliveryList, new SopComparer());
		//diff.Dump();
		...
		reports.Add(new SopReport { BranchCode = branch.Key, BranchName = branch.Value, TotalEs = esSopList.Count, TotalOal = oalDeliveryList.Count, MissingToDate = diff2.Count() });
	...
	if (response.StatusCode == System.Net.HttpStatusCode.OK)
	{
		...
		foreach (var deli in hits)
		{
			list.Add(new Sop
			{
				...
				DestOffice = deli["_source"]["DestOffice"],
...
private List<Sop> GetDeliveryEventNew(string officeNo, DateTime checkDate)
{
	...
	var sql = @"SELECT DISTINCT [consignment_no], [courier_id] FROM [dbo].[sop_event_new] WHERE office_no = '{0}' AND date_field > '{1:yyyy-MM-dd HH:mm:ss}' AND date_field < '{2:yyyy-MM-dd HH:mm:ss}'";
