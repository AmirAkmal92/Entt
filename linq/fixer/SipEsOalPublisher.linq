...
async Task Main()
{
	...
	foreach (var branch in branches)
	{
		...
		reports.Add(new SipReport { BranchCode = branch.Key, BranchName = branch.Value, TotalEs = esSipList.Count, TotalOal = toDateOalDeliveryList.Count, MissingToDate = diff2.Count() });
		...
		foreach (var sip in diff2)
		{
			var date = DateTime.ParseExact(sip.Date, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture);
			var time = DateTime.ParseExact(sip.Time, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture);

			var fileName = string.Format("sa_sip_{0:yyyyMMdd}_{1:HHmmss}_{2}.txt", date, time, sip.ScannerId);
			var data = string.Format("{0}\t{1}\t{2}\t{3:ddMMyyyy}\t{4:HHmmss}\t{5}\t{6}\r\n",
					sip.CourierId, sip.LocationId, sip.BeatNo, date, time, sip.ConfirmLocation, sip.ConsignmentNo);

			//Console.WriteLine(data);
			sb.Append(data);
			await PostSipData(rtsClient, fileName, data, productionToken);
		}
		var fn = string.Format("sa_sip_{0}_{1}_{2:yyyyMMdd}.txt", DateTime.Now.ToString("yyyyMMdd_hhmmss"), branch.Key, checkDate);
		File.AppendAllText(@"R:\script sa file\filedeli\Sip\" + fn, sb.ToString());
	}
...
private async Task PostSipData(HttpClient client, string filename, string data, string token)
{
	...
	var response = await client.PostAsync("sip", request);
...
private async Task<List<Sip>> GetEsSip(string elasticSearchUrl, string indexName, string officeNo, DateTime date)
{
	...
	if (response.StatusCode == System.Net.HttpStatusCode.OK)
	{
		...
		foreach (var sip in hits)
		{
			list.Add(new Sip
			{
				CourierId = sip["_source"]["CourierId"],
				LocationId = sip["_source"]["LocationId"],
				BeatNo = sip["_source"]["BeatNo"],
				Date = sip["_source"]["Date"],
				Time = sip["_source"]["Time"],
				ConfirmLocation = sip["_source"]["ConfirmLocation"],
				ConsignmentNo = sip["_source"]["ConsignmentNo"],
				ScannerId = sip["_source"]["ScannerId"]
			});
		}
	}
	return list;
