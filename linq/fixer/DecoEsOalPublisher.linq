...
async Task Main()
{
	...
	var reports = new List<DecoReport>();
	...
	foreach (var branch in branches)
	{
		...
		var toDateOalDeliveryList = GetToDateConsoleDetails(branch.Key, checkDate);
		var diff2 = esDecoList.Except(toDateOalDeliveryList, new DecoComparer());
		//diff2.Dump();

		reports.Add(new DecoReport { BranchCode = branch.Key, BranchName = branch.Value, TotalEs = esDecoList.Count, TotalOal = toDateOalDeliveryList.Count, MissingToDate = diff2.Count()});
		...
		foreach (var pick in diff2)
		{
			...
			var fileName = string.Format("sa_deco_{0:yyyyMMdd}_{1:HHmmss}_{2}.txt", date, time, pick.ScannerId);
			var data = string.Format("{0}\t{1}\t{2}\t{3:ddMMyyyy}\t{4:HHmmss}\t{5}\t{6}\t{7}\t{8}\r\n",
					pick.CourierId, pick.LocationId, pick.BeatNo, date, time, pick.ConsoleTag, pick.Comment, pick.AllConsignmentnNotes, pick.TotalConsignment);

			//Console.WriteLine(data);
			sb.Append(data);
			await PostDecoData(rtsClient, fileName, data, productionToken);
		}
		var fn = string.Format("sa_deco_{0}_{1}_{2:yyyyMMdd}.txt", DateTime.Now.ToString("yyyyMMdd_hhmmss"), branch.Key, checkDate);
...
private async Task PostDecoData(HttpClient client, string filename, string data, string token)
{
	...
	var response = await client.PostAsync("deco", request);
