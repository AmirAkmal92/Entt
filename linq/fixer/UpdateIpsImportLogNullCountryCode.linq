...
private List<IpsImport> GetNullCountryCodeList(DateTime date)
{
	...
	var sql = @"SELECT [id],[item_id] FROM [dbo].[ips_import_log] WHERE [orig_country_cd] IS NULL AND [event_date_local_date_field] > '{0:yyyy-MM-dd}' AND [event_date_local_date_field] < '{1:yyyy-MM-dd}'";
...
public async Task<int> UpdateIpsImportLog(IpsImport ips)
{
	var connString = @"Data Source=192.168.206.31;Initial Catalog=oal_dbo;Application Name=posentt;User Id=enttuser;password=P@ssw0rd";
	//var connString = @"Data Source=(localdb)\ProjectsV13;Initial Catalog=oal;Application Name=entt;Integrated Security=True";
	
	using (var conn = new SqlConnection(connString))
	using (var cmd = new SqlCommand(@"UPDATE [dbo].[ips_import_log] SET [orig_country_cd] = @orig_country_cd WHERE [id] = @id AND [item_id] = @item_id", conn))
	{
