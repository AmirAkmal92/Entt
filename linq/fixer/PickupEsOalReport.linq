...
async Task Main()
{
	...
	foreach (var branch in branches)
	{
		...
		//var diff = esDeliveryList.Except(oalDeliveryList, new PickupComparer());
		//diff.Dump();
		...
		reports.Add(new PickupReport { BranchCode = branch.Key, BranchName = branch.Value, TotalEs = esPickupList.Count, TotalOal = oalDeliveryList.Count, MissingToDate = diff2.Count()});	
...
private async Task<List<Pickup>> GetEsPickup(string elasticSearchUrl, string indexName, string officeNo, DateTime date)
{
	...
	if (response.StatusCode == System.Net.HttpStatusCode.OK)
	{
		...
		foreach (var deli in hits)
		{
			list.Add(new Pickup
			{
				ModuleId = deli["_source"]["ModuleId"],
				...
				AccountNo = deli["_source"]["AccountNo"],
				PickupNo = deli["_source"]["PickupNo"],
				ConsignmentNo = deli["_source"]["ConsignmentNo"],
				Postcode = deli["_source"]["Postcode"],
				ParentWeight = deli["_source"]["ParentWeight"],
				TotalItem = deli["_source"]["TotalItem"],
				ProductType = deli["_source"]["ProductType"],
				PackageType = deli["_source"]["PackageType"],
				Country = deli["_source"]["Country"],
				Height = deli["_source"]["Height"],
				Witdh = deli["_source"]["Witdh"],
				Length = deli["_source"]["Length"],
				ItemCategory = deli["_source"]["ItemCategory"],
				BabyConsignment = deli["_source"]["BabyConsignment"],
				TotalBaby = deli["_source"]["TotalBaby"],
				TotalParent = deli["_source"]["TotalParent"],
				BabyWeigth = deli["_source"]["BabyWeigth"],
				BabyHeigth = deli["_source"]["BabyHeigth"],
				BabyWidth = deli["_source"]["BabyWidth"],
				BabyLength = deli["_source"]["BabyLength"],
				RoutingCode = deli["_source"]["RoutingCode"],
				TotalWeight = deli["_source"]["TotalWeight"],
				TotalDimWeight = deli["_source"]["TotalDimWeight"],
				FailPickupReason = deli["_source"]["FailPickupReason"],
				Comment = deli["_source"]["Comment"],
				Price = deli["_source"]["Price"],
				ConsignmentFee = deli["_source"]["ConsignmentFee"],
				DropCode = deli["_source"]["DropCode"],
				LatePickup = deli["_source"]["LatePickup"],
				PlNine = deli["_source"]["PlNine"],
...
private List<Pickup> GetDeliveryEventNew(string officeNo, DateTime checkDate)
{
	...
	var sql = @"SELECT DISTINCT [consignment_no], [courier_id] FROM [dbo].[pickup_event_new] WHERE office_no = '{0}' AND date_field > '{1:yyyy-MM-dd HH:mm:ss}' AND date_field < '{2:yyyy-MM-dd HH:mm:ss}'";
