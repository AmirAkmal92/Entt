...
async Task Main()
{
	...
	var reports = new List<MissReport>();
	...
	foreach (var branch in branches)
	{
		...
		var esMissList = await GetEsMiss(elasticSearchUrl, indexName, branch.Key, checkDate);
		//esMissList.Dump();

		var toDateOalDeliveryList = GetToDateDeliveryEventNew(branch.Key, checkDate);
		var diff2 = esMissList.Except(toDateOalDeliveryList, new MissComparer());
		//diff2.Dump();

		reports.Add(new MissReport { BranchCode = branch.Key, BranchName = branch.Value, TotalEs = esMissList.Count, TotalOal = toDateOalDeliveryList.Count, MissingToDate = diff2.Count() });
		...
		foreach (var miss in diff2)
		{
			var date = DateTime.ParseExact(miss.Date, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture);
			var time = DateTime.ParseExact(miss.Time, "MM/dd/yyyy HH:mm:ss", System.Globalization.CultureInfo.InvariantCulture);

			var fileName = string.Format("sa_miss_{0:yyyyMMdd}_{1:HHmmss}_{2}.txt", date, time, miss.ScannerId);
			var data = string.Format("{0}\t{1}\t{2}\t{3:ddMMyyyy}\t{4:HHmmss}\t{5}\t{6}\r\n",
					miss.CourierId, miss.LocationId, miss.BeatNo, date, time, miss.ConfirmLocation, miss.ConsignmentNo);

			//Console.WriteLine(data);
			sb.Append(data);
			//await PostMissData(rtsClient, fileName, data, productionToken);
		}
		var fn = string.Format("sa_miss_{0}_{1}_{2:yyyyMMdd}.txt", DateTime.Now.ToString("yyyyMMdd_hhmmss"), branch.Key, checkDate);
		//File.AppendAllText(@"R:\script sa file\filedeli\miss\" + fn, sb.ToString());
	}
...
private async Task PostMissData(HttpClient client, string filename, string data, string token)
{
	...
	var response = await client.PostAsync("miss", request);
...
private async Task<List<Miss>> GetEsMiss(string elasticSearchUrl, string indexName, string officeNo, DateTime date)
{
	...
	var response = await client.PostAsync("miss/_search", request);
	...
	if (response.StatusCode == System.Net.HttpStatusCode.OK)
	{
		...
		foreach (var miss in hits)
		{
			list.Add(new Miss
			{
				CourierId = miss["_source"]["CourierId"],
				LocationId = miss["_source"]["LocationId"],
				BeatNo = miss["_source"]["BeatNo"],
				Date = miss["_source"]["Date"],
				Time = miss["_source"]["Time"],
				ConfirmLocation = miss["_source"]["ConfirmLocation"],
				ConsignmentNo = miss["_source"]["ConsignmentNo"],
				FaultReason = miss["_source"]["FaultReason"],
				OtherFaultReason = miss["_source"]["OtherFaultReason"],
				FaultLocation = miss["_source"]["FaultLocation"],
				NewPostCode = miss["_source"]["NewPostCode"],
				NewRoutingCode= miss["_source"]["NewRoutingCode"],
				ScannerId = miss["_source"]["ScannerId"]
			});
		}
	}
	return list;
...
private List<Miss> GetToDateDeliveryEventNew(string officeNo, DateTime checkDate)
{
	...
	var sql = @"SELECT DISTINCT [consignment_no], [courier_id] FROM [dbo].[missort_event_new] WHERE office_no = '{0}' AND date_field > '{1:yyyy-MM-dd HH:mm:ss}'";
	...
	using (var cmd = new SqlCommand(query, conn))
	{
		...
		using (var reader = cmd.ExecuteReader(CommandBehavior.CloseConnection))
		{
			while (reader.Read())
			{
				list.Add(new Miss { ConsignmentNo = reader.GetString(0), CourierId = reader.GetString(1) });
			}
		}
	}
	return list;
...
public class Miss
{
	...
	public string OtherFaultReason { get; set; }
	public string FaultLocation { get; set; }
	public string NewPostCode { get; set; }
	public string NewRoutingCode { get; set; }
...
public class MissComparer : IEqualityComparer<Miss>
{
	public bool Equals(Miss x, Miss y)
	{
		return x.ConsignmentNo == y.ConsignmentNo;
	}

	public int GetHashCode(Miss x)
	{
