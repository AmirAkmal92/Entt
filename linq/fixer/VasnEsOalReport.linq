...
async Task Main()
{
	...
	foreach (var branch in branches)
	{
		...
		//var diff = esDeliveryList.Except(oalDeliveryList, new VasnComparer());
		//diff.Dump();
		...
		reports.Add(new VasnReport { BranchCode = branch.Key, BranchName = branch.Value, TotalEs = esVasnList.Count, TotalOal = oalDeliveryList.Count, MissingToDate = diff2.Count() });
	...
	if (response.StatusCode == System.Net.HttpStatusCode.OK)
	{
		...
		foreach (var deli in hits)
		{
			list.Add(new Vasn
			{
				...
				ItemType = deli["_source"]["ItemType"],
				SenderName = deli["_source"]["SenderName"],
				CodAccount = deli["_source"]["CodAccount"],
				CodAmount = deli["_source"]["CodAmount"],
...
private List<Vasn> GetDeliveryEventNew(string officeNo, DateTime checkDate)
{
	...
	var sql = @"SELECT DISTINCT [consignment_no], [courier_id] FROM [dbo].[vasn_event_new] WHERE office_no = '{0}' AND date_field > '{1:yyyy-MM-dd HH:mm:ss}' AND date_field < '{2:yyyy-MM-dd HH:mm:ss}'";
