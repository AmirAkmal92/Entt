...
{
    public class SopScanSqlReceiveLocation : IReceiveLocation, IDisposable
    {
        ...
        private async void PollSopTableFired(object stateInfo)
        {
            if (m_paused) return;

            Console.WriteLine("now running Sop...");
            var logger = new LocationLogger();
            var number = 0;
            var records = await ReadSopAsync();
            ...
            foreach (var r in records)
            {
                ...
                //sow_Sop_<locationid>_<date>-<time>_<linecount>_<staffid>
                var filename = string.Format("sow_Sop_{0}_{1}_{2}_{3}", r.location_id, r.date_time.ToString("yyyyMMdd-HHmm"),1,r.courier_id);

                // polly policy goes here
                var retry = ConfigurationManager.GetEnvironmentVariableInt32($"{nameof(Sop)}Retry", 3);
                var interval = ConfigurationManager.GetEnvironmentVariableInt32($"{nameof(Sop)}Internal", 100);
                ...
                var pr = await Policy.Handle<Exception>()
                                    ...
                                    .ExecuteAndCaptureAsync(() =>
                    {
                        ...
                        return m_client.PostAsync("/api/rts/Sop", request);
                    });
        ...
        public async Task<int> InsertAsync(Sop item)
        {
            ...
            )", conn))
            {
                ...
                cmd.Parameters.Add("@pejabat_asal", SqlDbType.VarChar, 2000).Value = item.pejabat_asal;
                cmd.Parameters.Add("@next_location", SqlDbType.VarChar, 2000).Value = item.next_location;
                cmd.Parameters.Add("@berat", SqlDbType.VarChar, 2000).Value = item.berat;
                cmd.Parameters.Add("@routing_code", SqlDbType.VarChar, 2000).Value = item.routing_code;
        ...
        private async Task<IEnumerable<Sop>> ReadSopAsync()
        {
            using (var conn = new SqlConnection(ConfigurationManager.GetEnvironmentVariable("OalConnectionString")))
            //using (var conn = new SqlConnection(ConfigurationManager.GetEnvironmentVariable("ConnectionString")))
            ...
            {
                await conn.OpenAsync().ConfigureAwait(false);
                return await conn.QueryAsync<Sop>("SELECT TOP 5 * FROM [dbo].[Sop] WHERE [status] = '0'");
            }
        }
        ...
        public bool Start()
        {
            ...
            m_timer = new Timer(PollSopTableFired, flag, dueTime, period);
