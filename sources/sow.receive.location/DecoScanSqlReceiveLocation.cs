...
{
    public class DecoScanSqlReceiveLocation : IReceiveLocation, IDisposable
    {
        ...
        public async Task<int> InsertAsync(Deco item)
        {
            ...
            using (var cmd = new SqlCommand(@"INSERT INTO [dbo].[Deco_log] ([id],
                              [version],
                              [beat_no],
                              [comment],
                              [console_no],
        ...
        private async Task DeleteDecoRowAsync(Deco item)
        {
            using (var conn = new SqlConnection(ConfigurationManager.GetEnvironmentVariable("OalConnectionString")))
            //using (var conn = new SqlConnection(ConfigurationManager.GetEnvironmentVariable("ConnectionString")))
            using (SqlCommand cmd = new SqlCommand("DELETE FROM dbo.Deco WHERE Id=@Id", conn))
            {
        ...
        private async Task<IEnumerable<Deco>> ReadDecoAsync()
        {
            using (var conn = new SqlConnection(ConfigurationManager.GetEnvironmentVariable("OalConnectionString")))
            //using (var conn = new SqlConnection(ConfigurationManager.GetEnvironmentVariable("ConnectionString")))
            ...
            {
                await conn.OpenAsync().ConfigureAwait(false);
                return await conn.QueryAsync<Deco>("SELECT TOP 5 * FROM [dbo].[Deco] WHERE [status] = '0'");
            }
        }
        ...
        public bool Start()
        {
            ...
            m_timer = new Timer(PollDecoTableFired, flag, dueTime, period);
