...
{
    public class SipScanSqlReceiveLocation : IReceiveLocation, IDisposable
    {
        ...
        private async void PollSipTableFired(object stateInfo)
        {
            if (m_paused) return;

            Console.WriteLine("now running Sip...");
            var logger = new LocationLogger();
            var number = 0;
            var records = await ReadSipAsync();
            ...
            foreach (var r in records)
            {
                ...
                //sow_Sip_<locationid>_<date>-<time>_<linecount>_<staffid>
                var filename = string.Format("sow_Sip_{0}_{1}_{2}_{3}", r.location_id, r.date_time.ToString("yyyyMMdd-HHmm"),1,r.courier_id);

                // polly policy goes here
                var retry = ConfigurationManager.GetEnvironmentVariableInt32($"{nameof(Sip)}Retry", 3);
                var interval = ConfigurationManager.GetEnvironmentVariableInt32($"{nameof(Sip)}Internal", 100);
                ...
                var pr = await Policy.Handle<Exception>()
                                    ...
                                    .ExecuteAndCaptureAsync(() =>
                    {
                        ...
                        return m_client.PostAsync("/api/rts/sip", request);
                    });
        ...
        private async Task<IEnumerable<Sip>> ReadSipAsync()
        {
            using (var conn = new SqlConnection(ConfigurationManager.GetEnvironmentVariable("OalConnectionString")))
            //using (var conn = new SqlConnection(ConfigurationManager.GetEnvironmentVariable("ConnectionString")))
            ...
            {
                await conn.OpenAsync().ConfigureAwait(false);
                return await conn.QueryAsync<Sip>("SELECT TOP 5 * FROM [dbo].[Sip] WHERE [status] = '0'");
            }
        }
        ...
        public bool Start()
        {
            ...
            m_timer = new Timer(PollSipTableFired, flag, dueTime, period);
